...
void main (void)
{
    ...
    vec2 alpha = vec2(7.0, 7.0);
    vec3 H = normalize(N+L+E);
    float d = alpha.x*H.x*H.x + alpha.y*H.y*H.y;
    float brdf = min( 0.5*((1.0-d)*(cos(2.0*3.14*sqrt(d))*sin(2.0*3.14*sqrt(d))) + 1.0),  1.0);

    // write Total Color:
    gl_FragColor = (Idiff+Ispec)*vec4(1.0, 0.9, 0.5, 1.0) + 0.25*(brdf)*vec4(1.0, 1.0, 0.5, 1.0);
