...
class OpenGLCanvas(glcanvas.GLCanvas):
    ...
    def OnDraw(self, event):
        ...
        f0 = np.random.rand(1000,).astype('f')
        f1 = -f0;
        out = np.zeros(shape=(10000, ), dtype='f')
        input0ssbo = glGenBuffers(1)
        input1ssbo = glGenBuffers(1)
        outputssbo = glGenBuffers(1)

        self.cpsShader.setupShaderStorageBufferObject(input0ssbo, 0, f0)
        self.cpsShader.setupShaderStorageBufferObject(input1ssbo, 1, f1)
        self.cpsShader.setupShaderStorageBufferObject(outputssbo, 2, out)
        ...
        ptr = ctypes.cast(p, ctypes.POINTER(ctypes.c_float * len(out)))
