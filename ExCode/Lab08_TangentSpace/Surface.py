...
class Surface :
    ...
    def resetVerts(self):
        for i in range(0, self.nW) :
            for j in range(0, self.nH) :
                ...
                self.verts[j * self.nW + i, 1] = 0.05 * math.sin(20.5 * (x * z + z + x))
    ...
    def computeTangentSpace(self):
        ...
        #self.tangentBufferHandle = vboHandles[0]
        #self.binormBufferHandle = vboHandles[1]
        #glBindBuffer(GL_ARRAY_BUFFER, self.tangentBufferHandle)
        #glBufferData(GL_ARRAY_BUFFER, np.float32, self.tangent, GL_STATIC_DRAW)
        #glBindBuffer(GL_ARRAY_BUFFER, self.binormBufferHandle)
        #glBufferData(GL_ARRAY_BUFFER, self.binorm, GL_STATIC_DRAW)
        #vaoHandle = [0]
