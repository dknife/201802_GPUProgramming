...
class GAEngine:
    def __init__(self, nObstacles, nGenes):
        ...
        self.StartEndIdx = np.array([0,0], dtype=np.int32)
        ...
        self.StartEndIdxGPU = cuda.mem_alloc(self.StartEndIdx.size * self.StartEndIdx.dtype.itemsize)
    ...
    def setStartEndIdx(self, s, e):
        self.StartEndIdx = np.array([s,e], dtype=np.int32)
        cuda.memcpy_htod(self.StartEndIdxGPU, self.StartEndIdx)
    ...
    def rearrangePopulationWithRange(self, startIdx, endIdx):
        nThreads = endIdx - startIdx + 1
        func = self.mod.get_function("rearrangePopulationWithRange")
        threadsPerBlock = int(512)
        blocks = int(nThreads / threadsPerBlock + 1)
        func(self.geneGPU, self.fitGPU, self.StartEndIdxGPU, block=(threadsPerBlock, 1, 1), grid=(blocks, 1, 1))
    ...
    def update(self):
        ...
        up = self.nGenes - 1
        while up>0 :
            self.setStartEndIdx(0,up)
            self.rearrangePopulationWithRange(0, up)
            up = int(up/2)
    ...
    def draw(self):
        ...
        glVertexPointer(2, GL_FLOAT, 0, self.genes)
        glDrawArrays(GL_TRIANGLES, 0, 3 * self.nGenes)

        glLineWidth(5)
        glColor3f(0,1,0)
        glBegin(GL_LINE_LOOP)
        gene = self.genes[0 * 6:0 * 6 + 6]
        glVertex2fv(gene[0:2])
        glVertex2fv(gene[2:4])
        glVertex2fv(gene[4:6])
        glEnd()
